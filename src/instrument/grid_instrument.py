#https://github.com/dhilowitz/GridInstrument/blob/master/grid_instrument/grid_instrument.py
import time
import random
import math
import collections
...
class GridInstrument:
	...
	def _main_loop(self):
		...
		while True:
			...
			if but != []:
				...
				if x in [1, 2] and y == 9 and pressed and not self.kid_mode:
	...
	def get_currently_playing_midi_notes(self):
		...
		for buttonNumber in self._pressed_buttons:
			...
			y = (buttonNumber // 8) + 1  # Integer division for Python 3
			noteInfo = self._get_note_info(x, y)
	...
	def _grid_musical_mode_button_pressed(self, x, y):
		index = (x - 1) + ((4 - y) * 8)
		self._grid_musical_mode = list(self.MUSICAL_MODES.keys())[index]  # Convert keys view to list for Python 3
		if self.debugging:
